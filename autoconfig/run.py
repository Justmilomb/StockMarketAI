...
def _tail_progress_until_done(proc: subprocess.Popen, progress_file: Path) -> None:
    """Tail the .progress file for live fold updates while Claude works.

    Also captures Claude's final output (printed when session ends).
    """
    import threading

    claude_output_lines: list = []

    def _capture_claude_stdout() -> None:
        """Read Claude's stdout in background (buffered until session ends)."""
        for line in proc.stdout:
            claude_output_lines.append(line.rstrip())

    # Capture Claude's stdout in background
    stdout_thread = threading.Thread(target=_capture_claude_stdout, daemon=True)
    stdout_thread.start()

    # Tail .progress file until Claude finishes
    last_pos = 0
    ...
    while proc.poll() is None:
        ...
        # Show a spinner if no progress for 15+ seconds
        if time.time() - last_print_time > 15:
            spin_idx = (spin_idx + 1) % len(spinner_chars)
            elapsed = int(time.time() - last_print_time)
            print(f"\r  [{spinner_chars[spin_idx]}] Claude working... ({elapsed}s since last update)", end="", flush=True)

        time.sleep(1)

    # Final read of any remaining progress
    try:
        if progress_file.exists():
            with open(progress_file, "r", encoding="utf-8") as f:
                f.seek(last_pos)
    ...
    # Print Claude's summary (its final output from -p mode)
    if claude_output_lines:
        print(f"\n  {'─'*60}", flush=True)
        print(f"  Claude session summary:", flush=True)
        print(f"  {'─'*60}", flush=True)
        for line in claude_output_lines:
            if line.strip():
                print(f"  {line}", flush=True)
        print(flush=True)
