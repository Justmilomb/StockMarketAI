...
class SimpleWindow(QMainWindow):
    ...
    def _init_services(self) -> None:
        ...
        try:
            ...
            if self._claude_client:
                ...
                self._news_agent = NewsAgent(self._claude_client, refresh_interval_minutes=interval)
    ...
    def _build_ui(self) -> None:
        ...
        self._ai_label = QLabel(f"ai: {'ok' if ai_ok else 'off'}")
        self._ai_label.setStyleSheet(
            f"color: {COLORS['glow'] if ai_ok else COLORS['red']};"
            " font-weight: 400; padding: 0 8px; background: transparent;",
        )
        status.addPermanentWidget(self._ai_label)

        broker_live = self._broker_service and getattr(self._broker_service, "is_live", False)
        self._broker_label = QLabel("live" if broker_live else "paper")
        self._broker_label.setStyleSheet(
            f"color: {COLORS['glow'] if broker_live else COLORS['amber']};"
            " font-weight: 400; padding: 0 8px; background: transparent;",
        )
        status.addPermanentWidget(self._broker_label)

        self._time_label = QLabel("--")
        self._time_label.setStyleSheet(
            f"color: {COLORS['text_dim']}; padding: 0 8px; background: transparent;",
        )
    ...
    def _populate_cards(self) -> None:
        ...
        if not tickers:
            ...
            placeholder.setStyleSheet(
                f"color: {COLORS['text_dim']}; font-size: 14px; font-weight: 300;"
    ...
    def _refresh(self) -> None:
        ...
        self.statusBar().showMessage("refreshing...", 30000)
        self._run_background(self._fetch_data, self._on_data)

    def _fetch_data(self) -> Dict[str, Any]:
        """Background: fetch signals, prices, news."""
        result: Dict[str, Any] = {}
    ...
    @Slot(object)
    def _on_data(self, result: Dict[str, Any]) -> None:
        """Main thread: update cards with fresh data."""
        import pandas as pd

        if result.get("signals") is not None:
            self._signals = result["signals"]
        if result.get("live_data"):
            self._live_data.update(result["live_data"])
        if result.get("news"):
